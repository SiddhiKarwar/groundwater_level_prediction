...
base_csv = r"formalize_dataset\River Water Level\combined_river_water_level.csv"
...
# Load trained model
model_path = r"formalize_dataset\models\river_level_model.pkl"
model = joblib.load(model_path)

# Reference station data (using 'Darna dam' as baseline from sample)
ref_station = base_df[base_df['Station Name'] == 'Darna dam']
ref_times = pd.to_datetime(ref_station['Data Time'])
start_time = ref_times.min()
end_time = ref_times.max()
time_interval = ref_times.diff().mean()  # Approx interval
num_entries = len(ref_times)

# Prepare time series for missing locations
missing_locations = locations_df[locations_df['River Water Level'] == 0][['Latitude', 'Longitude']].copy()
all_predicted_data = []
...
updated_csv = os.path.join(output_folder, 'updated_combined_river_water_level.csv')
